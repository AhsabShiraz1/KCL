...
export default function KclConclusion({ setNotification, setCurrentLevel }: LevelProps) {
  ...
  const questions: Question[] = [
    {
      question: "Which vehicle picks up the stacks of cathode sheets and dispatch into the designated areas for transport?",
      options: ["Crane", "Moving robot", "Robotic Arm", "Forklift"],
      values: [0, 0, 0, 1],
    },
    {
      question: "What is the next step after anode is transported to cells?",
      options: ["Automated stripping", "Gassing", "Electrolysis", "Stacking"],
      values: [0, 0, 1, 0],
    },
    {
      question: "Where is the raw material at KCL taken first?",
      options: ["Project office", "Smelter", "Refinery", "Admin office"],
      values: [0, 1, 0, 0],
    },
  ];

  useEffect(() => {
    setNotification("This marks the end of KCL module. Please enter your session ID to begin.");
  ...
  const handleOptionClick = (index: number) => {
    ...
    localStorage.setItem(`KCL-${sessionId}-question${currentQuestionIndex + 1}`, isCorrect ? "1" : "0");
